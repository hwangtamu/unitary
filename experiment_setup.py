...
class Experiment(object):
    ...
    def check_attributes(self):
        ...
        if self.restrict_parameters:
            if self.d <= 7:
                print
                'WARNING: d is <= 7, but restrict parameters is true. Setting false.'
            if not 'general' in self.name:
                print
                'WARNING: restrict_parameters is only implemented for general unitary/orthogonal experiments. Setting false.'
...
def basis_test(d):
    ...
    for basis_change in bases:
        ...
        for lr in lrs:
            ...
            gen_basis = Experiment('general_unitary_basis' + str(basis_change) + '_lr' + str(lr), d,
                                   change_of_basis=basis_change)
