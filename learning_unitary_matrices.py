...
# helper functions
from unitary_utils import unitary_matrix, project_to_unitary, generate_unitary_learning
...
def numerical_gradient(loss_function, parameters, batch, pool,
    ...
    if update_indices is None:
        update_indices = range(len(parameters))
    ...
    if random_projections > 0:
        ...
        numerical_parallel = partial(numerical_random_gradient,
                                     ...
                                     batch=batch)
        if not pool is None:
            gradients_list = pool.map(numerical_parallel, range(N_RANDOM))
        else:
            # seemingly numpy will convert this to an array or something
            gradients_list = map(numerical_parallel, range(N_RANDOM))
...
def train_loop(experiment, train_batches, vali_batch, pool, loginfo):
    ...
    for (i, batch) in enumerate(train_batches):
        ...
        if i % MEASURE_SKIP == 0:
            ...
            loginfo['train_file'].write(
                exp_name + ' ' + str(t) + ' ' + str((i + 1) * batch_size) + ' ' + str(loss) + ' ' + str(
                    loginfo['rep']) + ' ' + loginfo['method'] + '\n')
            loginfo['vali_file'].write(
                exp_name + ' ' + str(t) + ' ' + str((i + 1) * batch_size) + ' ' + str(vali_loss) + ' ' + str(
                    loginfo['rep']) + ' ' + loginfo['method'] + '\n')
...
def logging(d, noise, batch_size, n_batches, start_from_rep, identifier=None):
    if identifier is None:
        logging_path = 'output/d' + str(d) + '_noise' + str(noise) + '_bn' + str(batch_size) + '_nb' + str(n_batches)
    else:
        assert type(identifier) == str
        logging_path = 'output/' + identifier + '_d' + str(d) + '_noise' + str(noise) + '_bn' + str(
            batch_size) + '_nb' + str(n_batches)
...
# === main loop === #
def main(d, experiments, identifier=None, n_reps=6, n_epochs=1, noise=0.01,
    ...
    # === outer rep loop! === #
    for rep in range(start_from_rep, start_from_rep + n_reps):
        ...
        if method == 'sparse':
            ...
            nonzero_index = sample(range(d * d), 1)[0]
            ...
            sparse_test.write('truth ' + map(lambda x: 'lambda_' + str(x), range(d * d)) + '\n')
        else:
