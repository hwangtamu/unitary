    ...
    if method == 'lie_algebra':
        if lambdas is None:
            ...
            if real:
                ...
                real_indices = [e for e in range(n * n) if e / n > e % n]
                lambdas[real_indices] = np.random.normal(size=(len(real_indices)))
                # real implies special, for the lie algebra method
            else:
                if special:
                    diagonal_indices = [e for e in range(n * n) if e / n == e % n]
                    non_diagonal_indices = [x for x in range(n * n) if not x in diagonal_indices]
                    # omit one randomly
